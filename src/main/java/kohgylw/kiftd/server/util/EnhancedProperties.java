...
import kohgylw.kiftd.server.util.EnhancedProperties.PropertiesContext.PropertyEntry;

/**
 * 
 * <h2>Properties的增强实现：在保存时能够保留原始文件结构</h2>
 * <p>
 * 该Properties的子类能够保存能够保持原文件结构顺序，便于程序修改后用户的阅读。同时由于使用了ArrayList替换HashSet作为存储结构，查询速度更快。
 * </p>
 * 
 * @author 青阳龙野(kohgylw)
 * @version 1.0
...
public class EnhancedProperties extends Properties {
	...
	public synchronized void store(OutputStream out, String header) throws IOException {
		...
		List<PropertyEntry> entrys = context.getCommentOrEntrys();
		for (Iterator<PropertyEntry> iter = entrys.iterator(); iter.hasNext();) {
	...
	class PropertiesContext {
		private List<PropertyEntry> commentOrEntrys = new ArrayList<>();

		public List<PropertyEntry> getCommentOrEntrys() {
			return commentOrEntrys;
		}

		public void addCommentLine(String line) {
			PropertyEntry pe = new PropertyEntry(null, null, line);
