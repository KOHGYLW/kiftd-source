...
### 常见问题
+ 更换更高版本的Spring Boot？
> 当您将kiftd所用的Spring Boot版本替换至更高（尤其是Spring Boot 2.0.4及以上版本）时，请重写源代码`kohgylw.kiftd.server.configation.MVC`类中的`afterPropertiesSet()`方法，并将其方法体留空。否则可能导致项目启动错误（特别鸣谢：用户 songdragon）。

+ 替换数据库时无法连接至MySQL？
> 当您按照《kiftd说明文档》中的说明将kiftd内置数据库替换为MySQL时可能出现无法连接的情况，这是由于内置驱动的时区与MySQL服务器的时区设置不匹配所导致的。如遇此问题，请修改源代码`kohgylw.kiftd.server.util.ConfigureReader`类中的`testServerPropertiesAndEffect`方法，将数据库链接声明（dbURL）参数后加入时区设置。示例：`dbURL = "jdbc:mysql://" + url + "？useUnicode=true&characterEncoding=utf8&serverTimezone=GMT%2B8";`，有关时区设置的详情可参见MySQL官方文档（特别鸣谢：用户 白衣胜雪）。
